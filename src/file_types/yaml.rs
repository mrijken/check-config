...
fn validate_regex(
    ...
    _validate_key_regex(&mut doc.as_mapping_mut().unwrap(), table_with_regex)
}
...
fn set(contents: &str, table_to_set: &toml::Table) -> Result<String, CheckError> {
    ...
    // todo convert unwrap
    _set_key_value(&mut doc.as_mapping_mut().unwrap(), table_to_set);
...
fn unset(contents: &str, table_to_unset: &toml::Table) -> Result<String, CheckError> {
    ...
    _remove_key(&mut doc.as_mapping_mut().unwrap(), table_to_unset);
